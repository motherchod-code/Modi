...
sock.ev.on("messages.upsert", async (m) => {
  ...
  // 🔥 cancel old process instantly
  if (cancelMap[jid]) {
    cancelMap[jid].cancelled = true;
  }
  ...
  const startFrom = CHANNEL_CONFIG[jid] || 185;

  runningMap[jid] = true;

  // ---------------- FAST SCAN ----------------
  for (let start = startFrom; start <= 10000; start += batchSize) {
    if (found || controller.cancelled) break;

    await Promise.allSettled(
      Array.from({ length: batchSize }, async (_, idx) => {
        if (controller.cancelled || found) return;
        ...
        try {
          ...
          if (!found) {
            found = true;
            console.log(`✅ INSTANT FOUND (${jid}) → ${id}`);
          }
  ...
  if (!found && !controller.cancelled) {
    ...
    while (!found && tries < 5 && !controller.cancelled) {
      tries++;
      console.log(`🚀 HYPER ${tries} → ${jid}`);

      await Promise.allSettled(
        Array.from({ length: 120 }, async () => {
          if (controller.cancelled || found) return;

          const id = String(startFrom + Math.floor(Math.random() * 800));
          const emoji = emojis[Math.floor(Math.random() * emojis.length)];
