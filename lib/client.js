...
function attachManagerEvents() {
  ...
  // Stores last 500 messages per session to recover deleted content
  const _msgCache = new Map(); // `${sessionId}:${msgId}` → { text, type, from, senderJid, pushName }
  ...
  manager.on("messages.upsert", async (sessionId, upsert) => {
    try {
      ...
      // ── Cache this message for antidelete recovery ────────────────────────────
      if (msg && msg.id && !raw?.message?.protocolMessage) {
        const cacheKey = `${sessionId}:${msg.id}`;
        _msgCache.set(cacheKey, {
          text:      msg.body || "",
          type:      msg.type || "",
          from:      msg.from || rawJid,
          senderJid: msg.sender || "",
          pushName:  msg.pushName || raw?.pushName || "Unknown",
          raw:       raw,
        });
      ...
      if (antidelete === true) {
        ...
        if (proto?.type === 0) {
          ...
          // Recover original message from cache
          const cached      = _msgCache.get(`${sessionId}:${deletedMsgId}`);
          const deletedText = cached?.text || "";
          const deletedType = cached?.type || "";
          const deleterName = cached?.pushName || senderNum;

          let notifyText =
            `🗑️ *AntiDelete Alert!*\n\n` +
            `👤 *Who:* @${senderNum} _(${deleterName})_\n` +
            `💬 *Chat:* ${deletedJid.endsWith("@g.us") ? "Group" : "Private"}\n`;

          if (deletedText) {
            notifyText += `\n📝 *Deleted Message:*\n${deletedText}`;
          } else if (deletedType) {
            notifyText += `\n📎 *Type:* ${deletedType.replace("Message", "")} (media)`;
          } else {
            notifyText += `\n📝 _(Message content not recoverable)_`;
          }
          ...
          // Re-send deleted media if available
          if (
            cached?.raw?.message &&
            deletedType &&
            deletedType !== "conversation" &&
            deletedType !== "extendedTextMessage" &&
            deletedType !== "protocolMessage"
          ) {
            try {
              const { downloadMediaMessage } = await import("@whiskeysockets/baileys");
              const buf = await downloadMediaMessage(cached.raw, "buffer", {});
              if (buf && buf.length > 0) {
                const mime = cached.raw.message?.[deletedType]?.mimetype || "";
                let mediaPayload = null;
                if (deletedType === "imageMessage")
                  mediaPayload = { image: buf, caption: "↑ Deleted image" };
                else if (deletedType === "videoMessage")
                  mediaPayload = { video: buf, caption: "↑ Deleted video" };
                else if (deletedType === "audioMessage")
                  mediaPayload = { audio: buf, mimetype: mime, ptt: cached.raw.message?.[deletedType]?.ptt || false };
                else if (deletedType === "stickerMessage")
                  mediaPayload = { sticker: buf };
                else if (deletedType === "documentMessage")
                  mediaPayload = { document: buf, mimetype: mime, fileName: cached.raw.message?.[deletedType]?.fileName || "file" };

                if (mediaPayload) {
                  await sock.sendMessage(notifyJid, mediaPayload).catch(() => {});
                }
              }
            } catch {}
          }
      ...
      // Also dispatch on sticker messages so antisticker plugin can handle them.
      const isSticker = !!(raw?.message?.stickerMessage);
      if ((body || isSticker) && !isStatus) {
